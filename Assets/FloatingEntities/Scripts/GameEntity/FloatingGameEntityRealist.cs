...
protected override void FixedUpdate()
{
  ...
  {
      // Check if the Rigidbody's rotation has a negative z value
      if (Mathf.Approximately(rb.rotation.z, -1f))
      {
          // Add an opposite rotation to compensate
          rb.rotation *= Quaternion.Euler(0f, 0f, 2f);
      }

      // Clamp the Z rotation of the Rigidbody between -25 and 25
      Vector3 eulerRotation = rb.rotation.eulerAngles;
      if (eulerRotation.z > 180f) {
          eulerRotation.z -= 360f;
      }
      eulerRotation.z = Mathf.Clamp(eulerRotation.z, -12.5f, 12.5f);
